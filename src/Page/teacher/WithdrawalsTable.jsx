...
import { CheckCircle, XCircle, Clock, Layers } from "lucide-react"; // Icons for actions
...
const WithdrawalsTable = () => {
    ...
    // Fetch withdrawals on component mount or page change
    useEffect(() => {
        const fetchWithdrawals = async () => {
            try {
                ...
                setWithdrawalStats(stats || { approved: 0, pending: 0, rejected: 0 }); // Default to zero if stats are missing
            } catch (err) {
    ...
    const handleUpdateStripeId = async (id, action) => {
        try {
            ...
            setWithdrawals((prevWithdrawals) =>
                prevWithdrawals.map((w) => (w._id === updatedWithdrawal._id ? updatedWithdrawal : w))
            );
        } catch (err) {
                        ...
                        <tbody>
                            {withdrawals.map((withdrawal) => (
                                <tr key={withdrawal._id} className="hover:bg-gray-50">
                                    <td className="border px-4 py-2">{withdrawal.teacher?.firstName + " " + withdrawal.teacher?.lastName || "N/A"}</td>
                                    ...
                                    <td className="border px-4 py-2">{new Date(withdrawal.requestedAt).toLocaleDateString()}</td>
                    ...
                    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-20">
                        <div className="bg-white p-6 rounded-md max-w-sm w-full">
                            ...
                            <div className="mt-4 flex justify-end space-x-2">
                                ...
                                <Button
                                    className={`px-4 py-2 ${currentAction === "approved" ? "bg-green-600 text-white" : "bg-red-600 text-white"} rounded-md hover:bg-green-700`}
                                    onClick={() => handleUpdateStripeId(currentWithdrawalId, currentAction)}
                                    disabled={loadingAction}
                                >
                                    {loadingAction ? (
                                        <><span className="animate-spin mr-2">🔄</span> {currentAction === "approved" ? "Approving..." : "Rejecting..."}</>
                                    ) : (
